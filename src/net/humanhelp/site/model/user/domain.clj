(ns net.humanhelp.site.model.user.domain
  "Pure rules for global HumanHelp User identity.
   ...
   verification state, account lifecycle, and canonical model commands.

   A User does not require a phone number or email address to exist.
   Authentication flows may impose stronger requirements without making a
   particular authentication mechanism part of the User domain.
...
(defn normalize-phone
  "Returns the canonical domain representation of an already-parsed phone
   number.

   Friendly/local formatting belongs outside the domain. User stores E.164."
  [value]
  (when
   (string? value)
    (let [value
...
(defn normalize-email
  "Returns HumanHelp's canonical case-insensitive email representation."
  [value]
  (when
   (string? value)
    (let [value
...
;; =============================================================================
;; Lifecycle values and public document facts
;; =============================================================================
...
(defn- within?
  [^Instant lower ^Instant value ^Instant upper]
  (and
   (at-or-before?
    lower
    value)
...
(defn- conventional-version-consistent?
  [user]
  (let [{:keys
         [created-at-key
          updated-at-key]}
...
(defn document-consistent?
  ...
  (and
   ...
   (conventional-version-consistent?
    user)
...
(defn- ensure-document!
  ...
  (ensure!
   ...
   {:user
    "The user document is internally inconsistent."}
...
(defn- update-user
  ...
  (let [changed
    ...
    (ensure!
     ...
     {:user
      "The operation would not change the user."}
...
(defn- ensure-changeable!
  ...
  (ensure!
   ...
   {:status
    "A deleted user cannot be changed."}
...
(defn- create-input-errors
  ...
  (cond-> {}
    ...
    (assoc
     :display-name
     (str
      "The display name must be at most "
      display-name-max
      " characters."))
    ...
    (assoc
     :phone-verified?
     "Phone verification must be true or false.")
    ...
    (assoc
     :email-verified?
     "Email verification must be true or false.")
...
(defn edit-profile
  ...
  (ensure!
   ...
   {:display-name
    "Use nil explicitly to remove the display name."}
...
(defn replace-phone
  ...
  (let [phone
    ...
    (ensure!
     ...
     {:phone
      "The phone must be canonical E.164."}
    ...
    (ensure!
     ...
     {:phone
      "The replacement matches the current phone."}
...
(defn replace-email
  ...
  (let [email
    ...
    (ensure!
     ...
     {:email
      "The replacement matches the current email."}
...
(defn remove-phone
  ...
  (ensure!
   ...
   {:phone
    "No phone is attached to the user."}
...
(defn remove-email
  ...
  (ensure!
   ...
   {:email
    "No email is attached to the user."}
...
(defn verify-phone
  ...
  (let [phone
    ...
    (ensure!
     ...
     {:phone
      "The verified phone must be canonical E.164."}
    ...
    (ensure!
     ...
     {:phone
      "The verification target is no longer current."}
    ...
    (ensure!
     ...
     {:phone
      "The current phone is already verified."}
...
(defn verify-email
  ...
  (let [email
    ...
    (ensure!
     ...
     {:email
      "The current email is already verified."}
