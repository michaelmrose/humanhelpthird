...
(defn document-consistent?
  ...
  (and
   ...
   (optional-uuid?
    (:user/suspended-by user))

   (optional-uuid?
    (:user/deleted-by user))

   (optional-reason?
    (:user/suspension-reason user))

   (optional-reason?
    (:user/deletion-reason user))
...
(defn normalize-create-input
  ...
  (let [input
    ...
    {:id
     ...
     :phone
     (user.common/normalize-phone
      (:phone input))

     :email
     (user.common/normalize-email
      (:email input))
...
(defn create-input-errors
  ...
  (cond-> {}
    ...
    (assoc
     :phone-verified?
     "A phone is required before verification.")
    ...
    (assoc
     :email-verified?
     "An email is required before verification.")
...
(defn remove-phone
  ...
  (ensure!
   ...
   {:contact
    "Add an email before removing the phone."})
...
(defn remove-email
  ...
  (ensure!
   ...
   :user/contact-required
   {:contact
    "Add a phone before removing the email."})
